...
## Testing Guidelines
Pester regression tests live in `tests/`; run `Invoke-Pester ./tests` when Pester is available. The required baseline validation path remains `./Merge-ModuleScripts.ps1`; contributors should treat a clean `Syntax OK` pass as the minimum gate before committing. After merge validation, run `./sign.ps1` against a local `sign.json` when changing runtime behavior for a provider or scheduler flow. Prefer real API-driven checks over HTML-only guesses when verifying dynamic pages or task completion.
